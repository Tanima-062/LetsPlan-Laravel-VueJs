    ...
    <div class="multi-select-filter" ref="multi_select_filter" v-click-outside="() => opened = false">
        ...
        <ul class="options" v-show="opened">
            <li class="option" v-for="option in options" :key="option[valueKey]">
                <label>
                    <input type="radio" :value="option[valueKey]" v-model="selectedItems" class="option_checkbox"   />
                    {{ option[nameKey] }}
                </label>
            </li>
        </ul>
...
export default {
    ...
    watch: {
        selectedItems: {
            handler: _debounce(async function (newValue, oldValue) {
                ...
                query[this.column] = this.selectedItems
    ...
    methods: {
        ...
        getSelectdOptionsFromUrl() {
            ...
            if(queries.hasOwnProperty(this.column)){
                keys.forEach(key => {
                    if (key == this.column && queries[key]) {
                        this.selectedItems = String(queries[key])
                    }
                })
            }else {
                console.log('get division options', this.options)
                let found = false;
