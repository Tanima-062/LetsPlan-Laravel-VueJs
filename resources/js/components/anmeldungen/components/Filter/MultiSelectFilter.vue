<template>
    <div class="multi-select-filter" ref="multi_select_filter" v-click-outside="() => opened = false">
        <div class="label" @click="toggle()" style="cursor: pointer">
            <span>{{ labelText }}</span>
            <DownArrowIcon />
        </div>
        <ul class="options" v-show="opened">
            <li v-for="option in options" :key="option[valueKey]" @click.stop="setSelectedItems(option[valueKey])"
                class="option">
                <label>
                    <Checkbox :value="option[valueKey]"
                        :checked="selectedItems.some(item => item == option[valueKey])" />
                    {{ option[labelKey] }}
                </label>
            </li>
        </ul>
    ...
    computed: {
        labelText() {
            if (this.selectedItems.length && this.options.length) {
                return (this.options.find(item => item[this.valueKey] == this.selectedItems[0])[this.labelKey]).concat(this.selectedItems.length > 1 ? `, +${this.selectedItems.length - 1}` : '')
            } else {
                return this.label
            }
        }
