<template>
    ...
    >
        <div
            class="label"
            :class="{ selected: $attrs.value, error: $attrs.error || errorMessage }"
            @click="toggle()"
        >
        ...
        <ul
            class="options"
            v-show="opened && options.length > 0"
        >
            <li
                v-for="option in options"
                :key="option[valueKey]"
                class="option"
                :class="{ selected: option[valueKey] == $attrs.value, 'not-selectable': !selectable(option) }"
...
.single-select-filter[disabled="disabled"] > .label{
    cursor: not-allowed;
}
</style>
