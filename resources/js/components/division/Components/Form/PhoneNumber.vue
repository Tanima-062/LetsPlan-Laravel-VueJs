...
export default {
    ...
    computed: {
        selectedCountry() {
            const selectedCountry = this.countries.find(country => country.dialCode == this.dialCode);
            if (selectedCountry) {
    ...
    methods: {
        updateDialCode(value) {
            ...
            this.$emit('update', { dialCode: value.dialCode, phoneNumber: this.phoneNumber })
            this.toggle()
        },

        updatePhoneNumber(e) {
            this.$emit('updatePhoneNumber', e.target.value.slice(0, 12))
            this.$emit('update', { dialCode: this.dialCode, phoneNumber: e.target.value.slice(0, 12) })
