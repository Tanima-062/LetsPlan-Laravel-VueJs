...
class AnmeldungenRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        $rules = [
            ...
            'membership_id' => "required_with:company_id",
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($this->spielgruppen_id && $this->spielgruppen_id !== 'keine') {
                ...
                if ($this->company_id) {
                    $division =  Division::where('company_id', $play_group->company_id)->where('status_id', 2)->first();
                }
                if ($this->division_id) {
